...
var = ['bc_a1_SRF','bc_a4_SRF','pom_a1_SRF','pom_a4_SRF','so4_a1_SRF',
       'so4_a2_SRF','so4_a3_SRF','soa_a1_SRF','soa_a2_SRF','AEROD_v']
conc_diff = np.zeros((10,192,288))
for m in range(len(var)):
    for n in range(len(Base)):
        conc_diff[m] +=  (np.reshape(AClean[n][var[m]].data,(192,288))*np.reshape(AClean[n]['PS'].data,(192,288))/np.reshape(AClean[n]['TS'].data,(192,288))/287.05*1e9
                          -np.reshape(Base[n][var[m]].data,(192,288))*np.reshape(Base[n]['PS'].data,(192,288))/np.reshape(Base[n]['TS'].data,(192,288))/287.05*1e9)
...
for m in range(len(var)):
    ...
    plt.savefig('F:/conc_diff_'+var[m]+'.png',  dpi=300)
    plt.show()
